...
var temp_name = get_string("Name to import","FNMLevel");

var temp_music_trans = get_open_filename("Music in ogg|*.ogg","music");

var music_temp = audio_create_stream(temp_music_trans);
...
var file_open = file_text_open_read(get_open_filename("fnf level chart|*.json","file"));
data = json_parse(file_text_read_string(file_open));
file_text_close(file_open);

save.bpm = data.song.bpm;

save.pistas = array_create(11,array_create(floor(save.duration * (data.song.bpm / 60) * 4)+1,0));
...
var temp_save_file = file_text_open_write(working_directory + "proyects\\" + temp_name + "\\data.json");

file_text_write_string(temp_save_file,json_stringify(save));

file_text_close(temp_save_file);

file_copy(temp_music_trans,working_directory + "proyects\\" + temp_name + "\\music.ogg")
