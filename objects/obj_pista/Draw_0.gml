...
//dibujar pista
draw_sprite_ext(sprite_index,0,frac(x / 48)*48,-jump_tracks*6,room_width/48+1,array_length(global.notes)/8,0,c_white,1);
...
for (var i = jump_tracks; i < jump_limit; i++) {
	for (var k = floor(abs(x/6)); k < array_length(global.notes[i]); k++) {
		...
		if global.interact and (global.notes[i][k] != -1) and point_in_rectangle(mouse_x,mouse_y,x+6*k,y+6*(i-jump_tracks),x+5+6*k,y+5+6*(i-jump_tracks)) {
			draw_set_alpha(0.1);
